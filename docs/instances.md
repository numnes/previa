...
| **ClickUp comments**                      | Optional. When a preview first becomes **active**, comment on the ClickUp task whose custom ID matches the branch name. Token + workspace ID in global Settings; enable per project. Instance detail shows task URL + status; you can paste a ClickUp URL to force-link without posting a comment. Linked tasks and dependencies are cached so the Instances list can search by related task ids. |
